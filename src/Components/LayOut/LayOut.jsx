    ...
    <Fragment>
      <nav className="fixed top-0 w-full flex items-center justify-between bg-colorOne dark:bg-colorTwo dark:text-colorOne text-colorTwo px-5 py-3 h-[10svh] z-50">
        <div className="flex flex-row justify-between items-center h-full">
          <div
            className={`w-[70px] md:w-[80px] h-full flex flex-col justify-center items-center gap-2 cursor-pointer ${isSideBarOpen ? "setIsSideBarOpen" : null
              }`}
            onClick={(e) => {
          ...
          >
            ...
            <span
              className={`w-[50px] md:w-[60px] h-[2px] dark:bg-colorOne bg-colorTwo nav-menu-bar-line-2 relative transition-all ${isSideBarOpen ? "-ml-5 " : "ml-5 "
                }`}
            ></span>
            <span className="w-[30px] md:w-[40px] h-[2px] dark:bg-colorOne bg-colorTwo"></span>
          </div>
      ...
      <div
        className={`dark:bg-colorTwo bg-colorOne dark:text-colorOne text-colorTwo p-4 h-[90svh] flex flex-col justify-between w-[80%] md:w-[50%] z-[22222222222222] fixed top-[10svh] bottom-0 left-0   transition-all ${isSideBarOpen ? "left-[0%]" : "left-[-80%]"
          }`}
      >
        <div>
          ...
          <div className="space-y-8 w-full flex flex-col">
            {sideBarsLinks.map((link, index) => (
              <React.Fragment key={index}>
                ...
                >
                  {link.name === 'Dashboard' ? <MdDashboard size={25} /> : link.name === 'Read Employees' ? <FaBookReader /> : link.name === 'Create Employee' ? <IoIosCreate /> : undefined}  {link.name}
                </NavLink>
              </React.Fragment>
      ...
      <div
        className={`w-full h-[100svh] bg-none bg-opacity-50 filter fixed z-30 bg-colorTwo ${isSideBarOpen ? "block" : "hidden"
          }`}
        onClick={() => {
          setIsSideBarOpen(!isSideBarOpen);
        }}
