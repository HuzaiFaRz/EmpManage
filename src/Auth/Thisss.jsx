...
const ImageUpload = () => {
  ...
  const uploadImage = async () => {
    ...
    data.append("cloud_name", cloudinaryConfig.uploadPreset);
    data.append("folder", "Cloudinary-React");

    try {
      const response = await fetch(
        `https://api.cloudinary.com/v1_1/${cloudinaryConfig.uploadPreset}/image/upload`,
        {
          method: "POST",
          body: data,
        }
        ...
        {loading ? (
          ...
          url && (
            <div className="pb-8 pt-4">
              <image src={cloudinaryConfig.uploadPreset} publicId={url} />
            </div>
          )
        )}
      </div>
